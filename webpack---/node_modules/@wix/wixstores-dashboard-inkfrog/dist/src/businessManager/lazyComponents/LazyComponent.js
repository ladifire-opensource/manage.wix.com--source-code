...
var LazyComponent = /** @class */ (function (_super) {
    ...
    function LazyComponent(props) {
        ...
        if (window.fedopsLogger.create) {
            var fedopsLogger = window.fedopsLogger.create('wixstores-dashboard-inkfrog');
            fedopsLogger.appLoadStarted();
        }
        var storeModule = business_manager_api_1.getModule(business_manager_api_1.ModuleId.Stores);
        var basicUrl = storeModule.topology.inkfrogStaticUrl;
        var debug = storeModule.getModuleParams().debug;
        var manifest = {
            files: [basicUrl + "registerComponent.bundle" + (debug ? '' : '.min') + ".js"],
            component: componentName_1.ComponentName.InkfrogPage,
            crossorigin: true,
        };
